...
namespace components::document_table::operators {
    ...
    operator_update::operator_update(services::collection::context_collection_t* context,
        ...
        : base::operators::read_write_operator_t(context, base::operators::operator_type::update)
    ...
    void operator_update::on_execute_impl(pipeline::context_t* pipeline_context) {
        // This implementation is almost identical to table::operators::operator_update
        ...
        if (left_ && left_->output() && right_ && right_->output()) {
            ...
            if (left_->output()->data_chunk().size() == 0 && right_->output()->data_chunk().size() == 0) {
                if (upsert_) {
                    output_ = base::operators::make_operator_data(base::operators::operator_t::context_->resource(), types_left);
                    ...
                    table::table_append_state state(base::operators::operator_t::context_->resource());
                    table().initialize_append(state);
                    for (size_t id = 0; id < output_->data_chunk().size(); id++) {
                        modified_->append(id + state.row_start);
                        base::operators::operator_t::context_->index_engine()->insert_row(output_->data_chunk(), id, pipeline_context);
                    }
                    table().append(output_->data_chunk(), state);
                }
            } else {
                modified_ = base::operators::make_operator_write_data<size_t>(base::operators::operator_t::context_->resource());
                no_modified_ = base::operators::make_operator_write_data<size_t>(base::operators::operator_t::context_->resource());
                output_ = base::operators::make_operator_data(left_->output()->resource(), types_left);
                auto state = table().initialize_update({});
                auto& out_chunk = output_->data_chunk();
                auto predicate = comp_expr_ ? table::operators::predicates::create_predicate(comp_expr_,
                                                                                              types_left,
                                                                                              types_right,
                ...
                for (size_t i = 0; i < chunk_left.size(); i++) {
                    for (size_t j = 0; j < chunk_right.size(); j++) {
                        if (predicate->check(chunk_left, chunk_right, i, j)) {
                            ...
                            base::operators::operator_t::context_->index_engine()->delete_row(chunk_left, i, pipeline_context);
                            bool modified = false;
                            for (const auto& expr : updates_) {
                            ...
                            base::operators::operator_t::context_->index_engine()->insert_row(chunk_left, i, pipeline_context);
                        }
                    }
                }
                out_chunk.set_cardinality(index);
                table().update(*state, out_chunk.row_ids, chunk_left);
            }
        } else if (left_ && left_->output()) {
            if (left_->output()->size() == 0) {
                if (upsert_) {
                    output_ = base::operators::make_operator_data(base::operators::operator_t::context_->resource(),
                ...
                auto state = table().initialize_update({});
                auto predicate =
                    comp_expr_ ? table::operators::predicates::create_predicate(comp_expr_, types, types, &pipeline_context->parameters)
                               : table::operators::predicates::create_all_true_predicate(left_->output()->resource());
                size_t index = 0;
                for (size_t i = 0; i < chunk.size(); i++) {
                    if (predicate->check(chunk, i)) {
                        ...
                        vector::validate_chunk_capacity(out_chunk, ++index);
                        base::operators::operator_t::context_->index_engine()->insert_row(chunk, i, pipeline_context);
                    }
                }
                out_chunk.set_cardinality(index);
                table().update(*state, out_chunk.row_ids, left_->output()->data_chunk());
            }
        }
    }
