    ...
    create_plan_delete(const context_storage_t& context,
        ...
        if (!is_document_table_storage(ctx)) {
            throw std::runtime_error(
                "create_plan_delete called for non-document_table collection: " +
        ...
        // Простой случай: delete с match (без JOIN)
        if (node_delete->collection_from().empty() && !node_raw_data) {
            auto plan = boost::intrusive_ptr(
                new components::document_table::operators::operator_delete(
                    context.at(node->collection_full_name()), nullptr));
            ...
            // Сложный случай: delete с JOIN или raw data
            // Используем full_scan + фильтр
            ...
            auto plan = boost::intrusive_ptr(
                new components::document_table::operators::operator_delete(
                    context.at(node->collection_full_name()), std::move(expr)));
            ...
            if (node_raw_data) {
                ...
                // DELETE с JOIN
                auto join_ctx = context.at(node_delete->collection_from());
                ...
                if (is_document_table_storage(join_ctx)) {
                    ...
                    components::logical_plan::node_ptr dummy_match_node;
                    // TODO: нужно создать правильный node или использовать table::planner
                    throw std::runtime_error(
                        "DELETE JOIN with non-document_table collection not yet supported");
                }
