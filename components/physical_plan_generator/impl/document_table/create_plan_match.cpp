...
namespace services::document_table::planner::impl {
    ...
    create_plan_match_(collection::context_collection_t* context_,
        ...
        if (is_document_table_storage(context_)) {
            auto scan = new components::document_table::operators::full_scan(context_, expr, limit);
            if (!projection.empty()) {
    ...
    create_plan_match(const context_storage_t& context,
        ...
        if (node->expressions().empty()) {
            // Scan без фильтра
            auto scan = new components::document_table::operators::full_scan(ctx, nullptr, limit);
            if (!projection.empty()) {
                scan->set_projection(projection);
            }
            return boost::intrusive_ptr(scan);
            ...
            auto expr = reinterpret_cast<const components::expressions::compare_expression_ptr*>(
                &node->expressions()[0]);
            return create_plan_match_(ctx, *expr, limit, projection);
        }
    }
