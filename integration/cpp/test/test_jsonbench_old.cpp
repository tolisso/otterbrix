...
TEST_CASE("OLD JSONBench 0a: BATCH INSERT Performance", "[jsonbench_old][batch_insert]") {
...
TEST_CASE("OLD JSONBench 0: INSERT Performance", "[jsonbench_old][insert]") {
    auto json_lines = read_ndjson_file(data_path);
    REQUIRE(!json_lines.empty());
    ...
    // Test 1: document_table with BATCH INSERT
    {
        std::cout << "[1/2] Testing document_table BATCH INSERT..." << std::endl;
        ...
        std::cout << "  ✓ document_table BATCH: " << dt_insert_time << "ms ("
...
TEST_CASE("OLD JSONBench Q1: Top event types", "[jsonbench_old][q1]") {
    auto json_lines = read_ndjson_file(data_path);
    REQUIRE(!json_lines.empty());
...
TEST_CASE("OLD JSONBench Q2: Event types with unique users", "[jsonbench_old][q2]") {
    auto json_lines = read_ndjson_file(data_path);
    REQUIRE(!json_lines.empty());
...
TEST_CASE("OLD JSONBench Q3: When do people use BlueSky (simplified)", "[jsonbench_old][q3]") {
    auto json_lines = read_ndjson_file(data_path);
    REQUIRE(!json_lines.empty());
...
TEST_CASE("OLD JSONBench Q4: First 3 users to post", "[jsonbench_old][q4]") {
    auto json_lines = read_ndjson_file(data_path);
    REQUIRE(!json_lines.empty());
...
TEST_CASE("OLD JSONBench Q5: Top 3 users with longest activity", "[jsonbench_old][q5]") {
    auto json_lines = read_ndjson_file(data_path);
    REQUIRE(!json_lines.empty());
